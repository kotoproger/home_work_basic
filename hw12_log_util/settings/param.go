...
func ParseParams(settings Settings) Settings {
	...
	pflag.StringVarP(&settings.OutputFileName, "output", "o", settings.OutputFileName, "Путь к файлу, в который будет записана статистика (необязательный флаг, если не указан, статистика выводится в стандартный поток вывода)")
	pflag.Parse()
